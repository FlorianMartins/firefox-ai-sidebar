...
- The submitted package corresponds to the git tag matching its `version`
  (e.g. version `1.23.10` → tag `v1.23.10`).
...
git checkout v1.23.10            # use the tag matching the submitted version
...
- `ai-sidebar-1.23.10-firefox.zip`  (Firefox — built from `manifest.json`)
- `ai-sidebar-chrome-1.23.10.zip`   (Chromium — built from `manifest.chrome.json`)
